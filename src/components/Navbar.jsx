import React, { useState } from 'react';
import SkillsConfirmModal from './SkillsConfirmModal';

const Navbar = ({ onHireMeClick }) => {
  const [isOpen, setIsOpen] = useState(false);
  const [showSkillsConfirm, setShowSkillsConfirm] = useState(false);

  const navLinks = [
    { name: 'Home', href: '#home' },
    { name: 'About Me', href: '#services' },
    { name: 'Skills', href: 'https://www.instagram.com/abel_jones_2007' },
    { name: 'Projects', href: 'https://github.com/AbelJonesMathew' },
    { name: 'Contact', href: '#contact' },
  ];

  return (
    <nav 
      className={`absolute top-0 left-0 w-full z-50 transition-all duration-300 ${
        isOpen ? 'bg-[#ff2a2a] py-4 shadow-xl' : 'bg-transparent py-6'
      }`}
    >
      <div className="max-w-7xl mx-auto px-6 md:px-12 flex justify-between items-center">
        
        {/* Left Side: Logo/Name */}
        <div className="flex items-center">
          <a href="#" className="text-white text-xl md:text-2xl font-black tracking-tight">
            Abel Jones Mathew<span className="text-red-500">.</span>
          </a>
        </div>

        {/* Center: Desktop Menu Links */}
        <div className="hidden md:flex space-x-8">
          {navLinks.map((link) => (
            <a 
              key={link.name} 
              href={link.href}
              target={link.name === 'Skills' ? undefined : (link.href.startsWith('http') ? '_blank' : '_self')}
              rel={link.name === 'Skills' ? undefined : (link.href.startsWith('http') ? 'noopener noreferrer' : undefined)}
              onClick={(e) => {
                if (link.name === 'Skills') {
                  e.preventDefault();
                  setShowSkillsConfirm(true);
                }
              }}
              className="text-white/80 hover:text-white font-medium relative group transition-colors duration-300 cursor-pointer"
            >
              {link.name}
              {/* Smooth hover underline */}
              <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-red-500 transition-all duration-300 group-hover:w-full"></span>
            </a>
          ))}
        </div>

        {/* Right Side: CTA Button */}
        <div className="hidden md:block">
          <a 
            href="#contact" 
            onClick={(e) => {
              e.preventDefault();
              if (onHireMeClick) onHireMeClick();
            }}
            className="px-6 py-2.5 rounded-full bg-white/10 border border-white/20 text-white font-semibold hover:bg-white/20 hover:shadow-[0_0_15px_rgba(255,255,255,0.2)] transition-all duration-300 backdrop-blur-md cursor-pointer"
          >
            Hire Me
          </a>
        </div>

        {/* Mobile Hamburger Menu Icon */}
        <div className="md:hidden flex items-center">
          <button 
            onClick={() => setIsOpen(!isOpen)}
            className="text-white focus:outline-none p-2"
          >
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              {isOpen ? (
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" />
              ) : (
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h16M4 18h16" />
              )}
            </svg>
          </button>
        </div>
      </div>

      {/* Mobile Slide-Down Menu */}
      <div 
        className={`md:hidden absolute top-full left-0 w-full transition-all duration-300 overflow-hidden ${
          isOpen ? 'max-h-96 py-4 opacity-100 bg-[#ff2a2a] shadow-2xl' : 'max-h-0 opacity-0 bg-transparent'
        }`}
      >
        <div className="flex flex-col px-6 space-y-4">
          {navLinks.map((link) => (
            <a 
              key={link.name} 
              href={link.href}
              target={link.name === 'Skills' ? undefined : (link.href.startsWith('http') ? '_blank' : '_self')}
              rel={link.name === 'Skills' ? undefined : (link.href.startsWith('http') ? 'noopener noreferrer' : undefined)}
              onClick={(e) => {
                if (link.name === 'Skills') {
                  e.preventDefault();
                  setIsOpen(false);
                  setShowSkillsConfirm(true);
                } else {
                  setIsOpen(false);
                }
              }}
              className="text-white hover:text-black font-bold text-lg border-b border-white/20 pb-2 transition-colors cursor-pointer"
            >
              {link.name}
            </a>
          ))}
          <div className="pt-4 pb-2">
             <a 
               href="#contact" 
               onClick={(e) => {
                 e.preventDefault();
                 setIsOpen(false);
                 if (onHireMeClick) onHireMeClick();
               }} 
               className="inline-block px-6 py-3 rounded-full bg-white text-[#ff2a2a] font-black hover:bg-black hover:text-white transition-colors w-full text-center shadow-lg cursor-pointer"
             >
               Hire Me
             </a>
          </div>
        </div>
      </div>

      <SkillsConfirmModal
        isOpen={showSkillsConfirm}
        onClose={() => setShowSkillsConfirm(false)}
        onProceed={() => {
          setShowSkillsConfirm(false);
          window.open('https://www.instagram.com/abel_jones_2007', '_blank', 'noopener,noreferrer');
        }}
      />
    </nav>
  );
};

export default Navbar;
